...
- 文稿列表使用单栏布局；搜索下方的“全部”菜单统一切换全部文稿、收藏夹、回收站及文件类型。退出编辑器时恢复目录、搜索、筛选和滚动位置。
- 文稿标题、筛选、面包屑与列表使用一致的页边距，滚动和返回列表后保持对齐。
- Tiyi 主应用在 iPad 常驻主侧栏，Tiyi 标题下的“文稿”直接打开列表；进入文件后切换全屏编辑，返回时恢复主侧栏。
- 任意层级文件夹、面包屑导航、同级名称冲突校验，以及名称、颜色和图标定制。
- 文件夹和文稿收藏、软删除、递归回收站、恢复、永久删除、清空回收站和批量移动。
- PDF 批量导入、重命名、移动和多文档 Tab。
- PDF 默认适宽显示，所有页面上下连续排列，页间保留窄间距；可停在两页之间，放大后仍能连续跨页滚动。页码跟随主要可见页面，缩略图和搜索结果可直接跳页；画板保留整屏分页。
- iPad PDF 单指滚动、双指缩放，Apple Pencil 书写；PDF 支持缩小至 10% 和越界回弹。双指缩放期间暂停单指滚动，所有手指离开后恢复。画板使用单指翻页、双指缩放和平移。
- 画板为无界工作区，始终铺满视口；双指缩放范围 50%–300%，缩小扩大可见范围，可向任意方向平移和书写。每张画板保留独立的缩放与位置，笔迹和对象使用稳定文稿坐标；PDF/PNG 导出包含原参考页范围外的内容。
- 新建无界画板包含空白、横线、方格和点阵背景，以及 6 种背景颜色；背景随世界坐标连续延伸。
- Mac Catalyst 可从 Finder 将一个或多个 PDF 拖入当前文件夹。
- iPad 支持圆珠笔、钢笔、铅笔、荧光笔、精细/整笔橡皮擦、撤销/重做、缩略图和逐页自动保存。
- 浮动笔工具条依次提供细笔、中笔、粗笔和荧光笔。再次点击荧光笔调整其粗细，点击任一笔粗细返回当前笔型；选择荧光笔时上排“笔”仍保持选中，再点上排“笔”打开笔型设置。
